...
typedef struct
{
	EXTI_GPIO_Mapping_t 	EXTI_PIN;    		// Specifies the EXTI GPIO mapping
									 	 	 	// This parameter must be set based on @ref EXTI define

	uint8_t 				TriggerCase;		// Specifies rising or falling or both.
												// This parameter must be set based on @ref EXTI_Trigger_case

	uint8_t					IRQ_EN;				// Enable or disable the EXTI_IRQ
												// That will enable the IRQ_MASK in EXTI and also on the NVIC
