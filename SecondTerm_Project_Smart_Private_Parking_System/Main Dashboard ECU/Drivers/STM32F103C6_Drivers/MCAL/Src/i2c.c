...
void MCAL_I2C_Init(I2C_TypeDef* I2Cx, I2C_Config_t* I2C_Config)
{
	...
	if(I2C_Config->I2C_Mode == I2C_MODE_ENABLE)
	{
		...
		tempreg = I2Cx->CR2;							/*	Get the I2Cx->CR2 Value   */

		tempreg &= ~(I2C_CR2_FREQ);						/* Clear frequency bits */

		Input_clk_freq = MCAL_RCC_GetPCLK1Freq(); 		/* Get PCLK1 frequency value */


		tempreg |= (Input_clk_freq / (_1MEGA));

		I2Cx->CR2 = tempreg;							/* Update the CR2 with the clock value */
		...
		{
			tempreg &= ~(1 << 15);						/* Configure as standard mode */

			CCR_Value = (Input_clk_freq/ (I2C_Config->I2C_ClockSpeed << 1));
			tempreg |= CCR_Value;

			I2Cx->CCR = tempreg;						/* Write to CCR Register */
...
void MCAL_I2C_Master_Tx(I2C_TypeDef *I2Cx, uint16_t devAdd, uint8_t* dataOut, uint32_t dataLen, stopCondition I2C_stop, repeatedStart Start)
{
	...
	while(!(I2C_GetFlagStatus(I2Cx, EV6))); /* Wait until the ADDR bit is set by hardware */
