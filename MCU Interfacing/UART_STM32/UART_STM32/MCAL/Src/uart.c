...
USART_Config_t *G_USART_CONFIG[3] = {NULL};
...
void UART_GPIO_Set_Pins (USART_TypeDef *USARTx)
{
	...
	if (USARTx == USART1)
	{
		...
		if (G_USART_CONFIG[0]->HwFlowCtl == UART_HwFlowCtl_CTS || G_USART_CONFIG[0]->HwFlowCtl == UART_HwFlowCtl_RTS_CTS)
		{
			//PA11 CTS
		...
		else if (G_USART_CONFIG[0]->HwFlowCtl == UART_HwFlowCtl_RTS || G_USART_CONFIG[0]->HwFlowCtl == UART_HwFlowCtl_RTS_CTS)
		{
			//PA12 RTS
	...
	else if (USARTx == USART2)
		{
			...
			if (G_USART_CONFIG[1]->HwFlowCtl == UART_HwFlowCtl_CTS || G_USART_CONFIG[1]->HwFlowCtl == UART_HwFlowCtl_RTS_CTS)
			{
				//PA0 CTS
			...
			else if (G_USART_CONFIG[1]->HwFlowCtl == UART_HwFlowCtl_RTS || G_USART_CONFIG[1]->HwFlowCtl == UART_HwFlowCtl_RTS_CTS)
			{
				//PA1 RTS
	...
	if (USARTx == USART3)
		{
			...
			if (G_USART_CONFIG[2]->HwFlowCtl == UART_HwFlowCtl_CTS || G_USART_CONFIG[2]->HwFlowCtl == UART_HwFlowCtl_RTS_CTS)
			{
				//PB13 CTS
			...
			else if (G_USART_CONFIG[2]->HwFlowCtl == UART_HwFlowCtl_RTS || G_USART_CONFIG[2]->HwFlowCtl == UART_HwFlowCtl_RTS_CTS)
			{
				//PB14 RTS
...
void MCAL_USART_Init(USART_TypeDef *USARTx, USART_Config_t* USART_Config)
{
	...
	if(USARTx == USART1)
		G_USART_CONFIG[0] = USART_Config;
	else if (USARTx == USART2)
		G_USART_CONFIG[1] = USART_Config;
	else if (USARTx == USART3)
		G_USART_CONFIG[2] = USART_Config;
...
void MCAL_USART_Send(USART_TypeDef *USARTx, uint16_t *pTxBuffer, Polling_mechism PollingEn)
{
	...
	if (USARTx == USART1)
	{
		if(G_USART_CONFIG[0]->Payload_Length == USART_Payload_9_bits)
		{
			USARTx->DR = (*pTxBuffer & (uint16_t)0x01FF);
		}
		else if (G_USART_CONFIG[0]->Payload_Length == USART_Payload_8_bits)
		{
		...
		{
			if(G_USART_CONFIG[1]->Payload_Length == USART_Payload_9_bits)
			{
				USARTx->DR = (*pTxBuffer & (uint16_t)0x01FF);
			}
			else if (G_USART_CONFIG[1]->Payload_Length == USART_Payload_8_bits)
			{
				USARTx->DR = (*pTxBuffer & (uint8_t)0xFF);
			}
		}
		...
		{
			if(G_USART_CONFIG[2]->Payload_Length == USART_Payload_9_bits)
			{
				USARTx->DR = (*pTxBuffer & (uint16_t)0x01FF);
			}
			else if (G_USART_CONFIG[2]->Payload_Length == USART_Payload_8_bits)
			{
				USARTx->DR = (*pTxBuffer & (uint8_t)0xFF);
			}
		}
	...
	if (USARTx == USART1)
		{
			...
			if(G_USART_CONFIG[0]->Parity != USART_Parity_disabled)
			{
				if (G_USART_CONFIG[0]->Payload_Length == USART_Payload_9_bits)
				{
			...
			else if (G_USART_CONFIG[0]->Parity == USART_Parity_disabled)
			{
				if (G_USART_CONFIG[0]->Payload_Length == USART_Payload_9_bits)
				{
			...
			{
				// if the parity bit is enabled
				if(G_USART_CONFIG[1]->Parity != USART_Parity_disabled)
				{
					if (G_USART_CONFIG[1]->Payload_Length == USART_Payload_9_bits)
					{
						*((uint16_t*) pRxBuffer) = ( USARTx->DR  & (uint8_t)0xFF );
				...
				// If there is no parity
				else if (G_USART_CONFIG[1]->Parity == USART_Parity_disabled)
				{
					if (G_USART_CONFIG[1]->Payload_Length == USART_Payload_9_bits)
					{
						*((uint16_t*) pRxBuffer) = USARTx->DR ;
			...
			{
				// if the parity bit is enabled
				if(G_USART_CONFIG[2]->Parity != USART_Parity_disabled)
				{
					if (G_USART_CONFIG[2]->Payload_Length == USART_Payload_9_bits)
					{
						*((uint16_t*) pRxBuffer) = ( USARTx->DR  & (uint8_t)0xFF );
				...
				// If there is no parity
				else if (G_USART_CONFIG[2]->Parity == USART_Parity_disabled)
				{
					if (G_USART_CONFIG[2]->Payload_Length == USART_Payload_9_bits)
					{
						*((uint16_t*) pRxBuffer) = USARTx->DR ;
...
void USART1_IRQHandler (void)
{
	...
	G_USART_CONFIG[0]->P_IRQ_CallBack () ;

}

void USART2_IRQHandler (void)
{
	G_USART_CONFIG[1]->P_IRQ_CallBack () ;

}
void USART3_IRQHandler (void)
{
	G_USART_CONFIG[2]->P_IRQ_CallBack () ;
